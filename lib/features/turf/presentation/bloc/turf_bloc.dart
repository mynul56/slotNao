...
import '../../domain/usecases/get_turf_detail_usecase.dart';
...
class TurfBloc extends Bloc<TurfEvent, TurfState> {
  ...
  Future<void> _onLoad(TurfLoadRequested event, Emitter<TurfState> emit) async {
    ...
    result.fold(
      ...
      (turfs) => emit(TurfListLoaded(turfs: turfs, hasMore: turfs.isNotEmpty)),
    );
  }

  Future<void> _onDetail(TurfDetailRequested event, Emitter<TurfState> emit) async {
    emit(const TurfLoading());
    final result = await _getTurfDetailUseCase(event.turfId);
    result.fold((failure) => emit(TurfError(failure.message)), (turf) => emit(TurfDetailLoaded(turf)));
  }

  Future<void> _onSearch(TurfSearchRequested event, Emitter<TurfState> emit) async {
    if (event.query.isEmpty) {
      add(const TurfLoadRequested());
      return;
    }
    emit(const TurfLoading());
    final result = await _searchTurfsUseCase(SearchTurfsParams(query: event.query));
    result.fold(
      (failure) => emit(TurfError(failure.message)),
      (results) => emit(TurfSearchResults(results: results, query: event.query)),
    );
  }
}
